...
public class SearchServiceImpl implements SearchService {
    ...
    private SearchResult buildSearchResult(SearchResponse response,SearchParam param) {
        ...
        List<SpuESTO> esModels = new ArrayList<>();
        //遍历所有商品信息
        if (hits.getHits() != null && hits.getHits().length > 0) {
            for (SearchHit hit : hits.getHits()) {
                String sourceAsString = hit.getSourceAsString();
                SpuESTO esModel = JSON.parseObject(sourceAsString, SpuESTO.class);
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                RPCResult<AttrRespVO> rpcResult = attrClient.attrInfo(Long.parseLong(s[0]));
                if (rpcResult.getCode() == 0) {
                    AttrRespVO data = rpcResult.getResult();
    ...
    private SearchRequest buildSearchRequest(SearchParam param) {
        ...
        SearchRequest searchRequest = new SearchRequest(new String[]{ESConstants.INDEX_PRODUCT},searchSourceBuilder);
